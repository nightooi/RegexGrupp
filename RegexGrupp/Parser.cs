...
namespace RegexGrupp
{
    internal class Parser
    {
        ...
        private static string _FindSpecific = "((<ASSERTDATE>))(\\s|\\W)*?(?<time>((0[0-9]|1[0-9]|2[0-3]):(\\d{0,2}):(\\d{0,2})))(?:(\\W*?))(?<sensorPos><POSITION>)(?:(\\W*?))(?<temp>(\\-*?)\\d{0,2}\\.\\d{0,2})(?:(\\W*?))(?<humidity>\\d{0,2})(\\s|\\W)*?$";
        ...
        private Func<IEnumerable<string>, MatchCollection> BuildEndo(string position, string data)
        {
           ...
           var searchPattern = Regex.Replace(_FindSpecific, $"({_Position})", Insertion);
            var searchPatt = Regex.Replace(searchPattern, $"{_dateAssert}", _AssertDateRange);
            Regex regex = new Regex(searchPatt, RegexOptions.Multiline, TimeSpan.FromSeconds(10));
